...
from temporal.pydanticai.codeact.datamodels.mcp_tools import ModelSerializedMcp, ModelSerializedMcpAdapter
...
def __json_schema_type_to_python(
    ...
    if json_type == "object":
        ...
        if additional:
            value_type = __json_schema_type_to_python(additional, required=True) if isinstance(additional,
                                                                                               dict) else "Any"
...
async def extract_mcp_tools_as_functions(
        mcp_configs: list[ModelSerializedMcp],
) -> list[str]:
    ...
    servers = []
    mcp_server_configs = ModelSerializedMcpAdapter.dump_python(mcp_configs)
    for config in mcp_server_configs:
        server_type = config.pop('kind', 'stdio')
        if server_type == 'stdio':
            ...
            raise ApplicationError(message=f"WARNING: Unsupported MCP server type: {server_type}", non_retryable=True)
